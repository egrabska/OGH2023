...
```{r message=FALSE, warning=FALSE}
df_inter = df_tsb %>% 
  model(arima = ARIMA(index_out ~ -1 + pdq(0,1,0) + PDQ(0,0,0))) %>% 
  fabletools::interpolate(df_tsb)
```

Apply Savitzky-Golay filter. n is a filter length (odd number) - test different values.
```{r message=FALSE, warning=FALSE}
df_inter$sg = df_inter$index_out %>%
  sgolayfilt(n = 71) 
```  
...
df_inter$original = df_tsb$index_out
ggplot(df_inter)+
  geom_point(aes(date, index_out), color = "blue", alpha = 0.1)+
...
```{r message=FALSE, warning=FALSE}
model = gamm(df_tsb$index_out ~ s(date, k = 60), 
             data = df_tsb, method = "REML")
```

Predicting values using GAM model and plotting the results:
```{r message=FALSE, warning=FALSE}
df_tsb$predicted = predict.gam(model$gam, df_tsb)
df_tsb$date =  anydate(df_tsb$date)
